...
const HomeAiPage = () => {
    ...
    const renderContent = () => {
        if (isLoading) {
            return _jsx("p", { className: "text-center p-8 text-slate-500 dark:text-slate-400", children: "Loading AI modules..." });
        }
        if (error) {
            return _jsx("p", { className: "text-center p-8 text-red-500", children: error });
        }
        if (flatTemplates.length === 0) {
            return _jsx("p", { className: "text-center p-8 text-slate-500 dark:text-slate-400", children: "No AI modules are available." });
        }
