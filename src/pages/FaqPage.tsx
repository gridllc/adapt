...
const FaqPage: React.FC = () => {
    ...
    const { data: tutorLogs = [], isLoading: isLoadingLogs } = useQuery<TutorLogRow[], Error>({
        ...
        queryFn: () => {
            if (!selectedModule?.slug) return []; // Should not happen due to 'enabled', but for type safety.
                ...
                <div className="mb-6">
                    ...
                    >
                        ...
                        {!isLoadingModules && availableModules.map(module => (
                            module.slug && <option key={module.slug} value={module.slug}>{module.title}</option>
                        ))}
