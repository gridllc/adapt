...
import type { FlaggedQuestion, TutorLogRow } from '@/types';
...
const QuestionLogDetailPage: React.FC = () => {
    ...
    const flaggedLogIds = useMemo(() => {
        return new Set(flaggedQuestions.map((f: FlaggedQuestion) => f.tutorLogId).filter(Boolean));
    }, [flaggedQuestions]);
                        ...
                        {aiSuggestion.sourceQuestions && aiSuggestion.sourceQuestions.length > 0 && (
                            <div className="mt-3 pt-3 border-t border-yellow-300 dark:border-yellow-600/50">
                                ...
                                <ul className="mt-1 space-y-1">
                                    {aiSuggestion.sourceQuestions.slice(0, 2).map((q: string, i: number) => (
