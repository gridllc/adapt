...
export const getQuestionFrequency = async (moduleId) => {
    const url = `/analytics/question-frequency?moduleId=${encodeURIComponent(moduleId)}`;
    return authedApiFetch(url);
};
export const getTutorLogs = async (moduleId) => {
    const url = `/analytics/tutor-logs?moduleId=${encodeURIComponent(moduleId)}`;
    return authedApiFetch(url);
};
export const getAllTutorLogs = async () => {
    return authedApiFetch('/analytics/tutor-logs/all');
};
...
export const findHotspots = (stats, module) => {
...
export const findPlatformHotspot = (allLogs, allModules) => {
    if (allLogs.length === 0)
        return null;
    const logCounts = {};
    allLogs.forEach(log => {
