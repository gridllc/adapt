...
export interface DetectedAlias {
    alias: string; // e.g. "circle thingy"
    formalName: string; // e.g. "Navigation Ring"
}

// A simple, expandable map for common user phrases.
// In a production system, this could be loaded from a database.
export const buttonAliasMap: Record<string, string> = {
 ...
 */
export function detectAliases(userQuestion: string): DetectedAlias[] {
  const matches: DetectedAlias[] = [];
 ...
 */
export function getAliasPromptInjection(): string {
