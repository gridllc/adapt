...
const EventIcon = ({ type }) => {
    switch (type) {
        case 'hint': return <span title="Hint Provided"><Icons_1.LightbulbIcon className="h-5 w-5 text-yellow-400"/></span>;
        case 'correction': return <span title="Correction Made"><Icons_1.AlertTriangleIcon className="h-5 w-5 text-red-400"/></span>;
        case 'tutoring': return <span title="Tutoring Session"><Icons_1.LightbulbIcon className="h-5 w-5 text-orange-400"/></span>;
        default: return <span title="Step Advanced"><Icons_1.ClockIcon className="h-5 w-5 text-slate-400"/></span>;
    }
};
/**
...
const SessionReviewPage = () => {
    const { moduleId, session_key } = (0, react_router_dom_1.useParams)();
    // Fetch the core module data (title, steps, etc.)
    const { data: moduleData, isLoading: isLoadingModule } = (0, react_query_1.useQuery)({
    ...
    const { data: sessionSummary, isLoading: isLoadingSession } = (0, react_query_1.useQuery)({
        queryKey: ['liveCoachSessionSummary', moduleId, session_key],
        queryFn: () => (0, sessionService_1.getSessionSummary)(moduleId, session_key),
    ...
    return (<div className="p-8">
            ...
            <div className="bg-white dark:bg-slate-800/50 p-8 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-center">
                    <div className="bg-slate-200 dark:bg-slate-900/50 p-4 rounded-lg">
                        ...
                        <div className="flex items-center justify-center gap-2 text-4xl font-bold mt-2">
                            <Icons_1.TrophyIcon className="h-10 w-10 text-yellow-400"/>
                    ...
                    <div className="bg-slate-200 dark:bg-slate-900/50 p-4 rounded-lg">
                        ...
                        <div className="flex items-center justify-center gap-2 text-4xl font-bold mt-2">
                            <Icons_1.ClockIcon className="h-10 w-10 text-slate-500 dark:text-slate-400"/>
                ...
                <div>
                    ...
                    {Object.keys(sessionSummary.durationsPerStep).length > 0 ? (<div className="space-y-2">
                            {Object.entries(sessionSummary.durationsPerStep).map(([stepIndex, duration]) => (<div key={stepIndex} className="flex justify-between items-center bg-white dark:bg-slate-700/50 p-3 rounded-md text-sm">
                        ...
                        </div>) : (<p className="text-center text-sm text-slate-500 py-4">No step duration data was recorded.</p>)}
                </div>

                {/* --- Full Event Timeline --- */}
                <div>
                    <h2 className="text-xl font-bold text-indigo-500 dark:text-indigo-400 mb-6">Event Timeline</h2>
                    {events.length > 0 ? (<div className="flow-root">
                            <ul className="-mb-8">
                                {events.map((event, eventIdx) => (<li key={eventIdx}>
                                        <div className="relative pb-8">
                                            {eventIdx !== events.length - 1 ? (<span className="absolute top-4 left-4 -ml-px h-full w-0.5 bg-slate-200 dark:bg-slate-700" aria-hidden="true"/>) : null}
                        ...
                        </div>) : (<div className="text-center text-slate-500 bg-slate-200 dark:bg-slate-900/50 p-6 rounded-lg">
...
exports.default = SessionReviewPage;
