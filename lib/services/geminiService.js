...
exports.generateBranchModule = exports.generateImage = exports.evaluateCheckpointAnswer = exports.generatePerformanceSummary = exports.getFallbackResponse = exports.sendMessageWithRetry = exports.startChat = exports.detectIntent = exports.generateModuleFromModelNumber = exports.generateModuleFromContext = exports.getTranscriptWithConfidence = void 0;
...
function getAiClient() {
    ...
    cachedClient = new genai_1.GoogleGenAI({ apiKey });
    return cachedClient;
}
    ...
    properties: {
        ...
        transcript: {
            ...
            items: {
                type: genai_1.Type.OBJECT,
                properties: {
                    start: { type: genai_1.Type.NUMBER, description: "Start time of the speech segment in seconds." },
                    end: { type: genai_1.Type.NUMBER, description: "End time of the speech segment in seconds." },
                    text: { type: genai_1.Type.STRING, description: "The transcribed text for this segment, with filler words like 'um' or 'uh' removed." },
                },
...
const moduleFromTextSchema = {
    type: genai_1.Type.OBJECT,
    properties: {
        slug: { type: genai_1.Type.STRING, description: "A URL-friendly slug for the module, based on the title (e.g., 'how-to-boil-water')." },
        title: { type: genai_1.Type.STRING, description: "A concise, descriptive title for the overall process." },
        steps: {
            type: genai_1.Type.ARRAY,
            ...
            items: {
                type: genai_1.Type.OBJECT,
                properties: {
                    start: { type: genai_1.Type.NUMBER, description: "The start time of this step in seconds. Set to 0 as a placeholder." },
                    end: { type: genai_1.Type.NUMBER, description: "The end time of this step in seconds. Set to 0 as a placeholder." },
                    title: { type: genai_1.Type.STRING, description: "A short, action-oriented title for the step (e.g., 'Toast the Bread')." },
                    description: { type: genai_1.Type.STRING, description: "A detailed explanation of how to perform this step." },
                    checkpoint: { type: genai_1.Type.STRING, nullable: true, description: "A question to verify the trainee's understanding of this step. Should be null if not applicable." },
                    alternativeMethods: {
                        type: genai_1.Type.ARRAY,
                        description: "Optional alternative ways to perform the step. Should be an empty array if not applicable.",
                        items: { type: genai_1.Type.OBJECT, properties: { title: { type: genai_1.Type.STRING }, description: { type: genai_1.Type.STRING } }, required: ["title", "description"] }
                    }
                },
 ...
 */
const getTranscriptWithConfidence = async (videoFile) => {
    if (import.meta.env.DEV)
        console.time('[AI Perf] getTranscriptWithConfidence');
...
The output MUST be a single, valid JSON object adhering to the provided schema.`;
    try {
        ...
        if (import.meta.env.DEV)
            console.timeEnd('[AI Perf] getTranscriptWithConfidence');
    }
};
exports.getTranscriptWithConfidence = getTranscriptWithConfidence;
/**
 ...
 */
const generateModuleFromContext = async (context) => {
    if (import.meta.env.DEV)
        console.time('[AI Perf] generateModuleFromContext');
    const ai = getAiClient();
    ...
    **Additional Notes:** ${context.notes || "No additional notes provided."}
    ...
    try {
        ...
        if (!text)
            throw new Error("AI returned empty response for module generation.");
        ...
        if (import.meta.env.DEV)
            console.timeEnd('[AI Perf] generateModuleFromContext');
    }
};
exports.generateModuleFromContext = generateModuleFromContext;
/**
 ...
 * @returns A promise that resolves to the generated module data.
 */
const generateModuleFromModelNumber = async (brand, model) => {
    if (import.meta.env.DEV)
        console.time('[AI Perf] generateModuleFromModelNumber');
    const ai = getAiClient();
    ...
    **Device Model:** ${model}
    ...
    try {
        ...
        if (import.meta.env.DEV)
            console.timeEnd('[AI Perf] generateModuleFromModelNumber');
    }
};
exports.generateModuleFromModelNumber = generateModuleFromModelNumber;
 ...
 */
const detectIntent = async (userQuestion) => {
    if (import.meta.env.DEV)
        console.time('[AI Perf] detectIntent');
    const ai = getAiClient();
    ...
    try {
        ...
        if (import.meta.env.DEV)
            console.timeEnd('[AI Perf] detectIntent');
    }
};
exports.detectIntent = detectIntent;
function getChatTutorSystemInstruction(stepsContext, fullTranscript, templateContext) {
    ...
    baseInstruction += `${(0, aliasService_1.getAliasPromptInjection)()}\n\n`;
    if (templateContext) {
        ...
        if (templateContext.buttons && templateContext.buttons.length > 0) {
            ...
            templateContext.buttons.forEach((btn) => {
 ...
 */
const startChat = (stepsContext, fullTranscript, history = [], templateContext) => {
    const ai = getAiClient();
 ...
 */
async function sendMessageWithRetry(chat, prompt, retries = 2) {
 ...
 */
const getFallbackResponse = async (prompt, history, stepsContext, fullTranscript) => {
    if (import.meta.env.DEV)
        console.time('[AI Perf] getFallbackResponse');
    ...
    const geminiHistory = history.slice(-20).map(msg => ({ role: msg.role, parts: [{ text: msg.text }] }));
    ...
    try {
        ...
        if (import.meta.env.DEV)
            console.timeEnd('[AI Perf] getFallbackResponse');
    }
};
 ...
 */
const generatePerformanceSummary = async (moduleTitle, unclearSteps, userQuestions) => {
    if (import.meta.env.DEV)
        console.time('[AI Perf] generatePerformanceSummary');
    const ai = getAiClient();
    ...
    try {
        ...
        const text = response.text;
        if (!text)
            throw new Error("AI returned empty response for performance summary.");
        return parseJson(text);
    }
    catch (error) {
        ...
        if (import.meta.env.DEV)
            console.timeEnd('[AI Perf] generatePerformanceSummary');
    }
};
...
const checkpointEvaluationSchema = {
    type: genai_1.Type.OBJECT,
    properties: {
        isCorrect: { type: genai_1.Type.BOOLEAN, description: "Whether the user's answer correctly answers the checkpoint question based on the step description." },
        feedback: { type: genai_1.Type.STRING, description: "A brief, one-sentence explanation for why the answer is correct or incorrect." },
        suggestedInstructionChange: { type: genai_1.Type.STRING, nullable: true, description: "If the user's answer is wrong but logical, suggest a rewrite of the original step description to prevent this confusion. Otherwise, null." }
    },
 ...
 */
const evaluateCheckpointAnswer = async (step, answer) => {
    if (import.meta.env.DEV)
        console.time('[AI Perf] evaluateCheckpointAnswer');
    const ai = getAiClient();
    ...
    try {
        ...
        const text = response.text;
        if (!text)
            throw new Error("AI returned empty response for checkpoint evaluation.");
        return parseJson(text);
    }
    catch (error) {
        ...
        if (import.meta.env.DEV)
            console.timeEnd('[AI Perf] evaluateCheckpointAnswer');
    }
};
exports.evaluateCheckpointAnswer = evaluateCheckpointAnswer;
...
const generateImage = async (prompt) => {
    if (import.meta.env.DEV)
        console.time('[AI Perf] generateImage');
    const ai = getAiClient();
    try {
        ...
        if (import.meta.env.DEV)
            console.timeEnd('[AI Perf] generateImage');
    }
};
...
const branchModuleSchema = {
    type: genai_1.Type.OBJECT,
    properties: {
        title: { type: genai_1.Type.STRING, description: "A short, descriptive title for the new remedial mini-module (e.g., 'Troubleshooting the Scanner')." },
        steps: {
            type: genai_1.Type.ARRAY,
 ...
 */
const generateBranchModule = async (stepTitle, questions) => {
    if (import.meta.env.DEV)
        console.time('[AI Perf] generateBranchModule');
    const ai = getAiClient();
    ...
    try {
        ...
        const text = response.text;
        if (!text)
            throw new Error("AI returned empty response for branch module generation.");
        return parseJson(text);
    }
    catch (error) {
        ...
        if (import.meta.env.DEV)
            console.timeEnd('[AI Perf] generateBranchModule');
    }
};
exports.generateBranchModule = generateBranchModule;
