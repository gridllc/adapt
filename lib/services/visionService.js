...
exports.isObjectPresent = exports.detectObjectsInVideo = exports.initializeObjectDetector = void 0;
...
const initializeObjectDetector = async () => {
    ...
    try {
        const vision = await tasks_vision_1.FilesetResolver.forVisionTasks("https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@latest/wasm");
        objectDetector = await tasks_vision_1.ObjectDetector.createFromOptions(vision, {
            baseOptions: {
...
exports.initializeObjectDetector = initializeObjectDetector;
/**
 ...
 */
const detectObjectsInVideo = (videoElement) => {
    if (!objectDetector) {
        return [];
    }
...
exports.detectObjectsInVideo = detectObjectsInVideo;
/**
 ...
 */
const isObjectPresent = (detectedObjects, targetObject) => {
    if (!targetObject)
        return false;
    const target = targetObject.toLowerCase();
    const possibleLabels = [target, ...(synonymMap[target] || [])];
    return detectedObjects.some(detected => possibleLabels.some(label => detected.label.toLowerCase().includes(label)));
};
