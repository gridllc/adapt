// gemini-functions/src/routes/chatRoutes.ts
import express from "express";
import { db, admin } from "../firebase"; // Use centralized admin
import { authed, AuthedRequest } from "../middleware/authed";

const router = express.Router();

// --- Public Routes ---
router.get("/", async (req: express.Request, res: express.Response) => {
    const { moduleId, sessionToken } = (req as any).query;
    if (!moduleId || !sessionToken) {
        return (res as any).status(400).json({ error: "moduleId and sessionToken are required." });
    }
    const snap = await db.collection("chatMessages").where("module_id", "==", moduleId as string).where("session_token", "==", sessionToken as string).orderBy("created_at").get();
    return (res as any).status(200).json(snap.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
});

router.post("/", async (req: express.Request, res: express.Response) => {
    const { moduleId, sessionToken, message } = (req as any).body;
    if (!moduleId || !sessionToken || !message?.id) {
        return (res as any).status(400).json({ error: "moduleId, sessionToken, and a message with an ID are required." });
    }
    await db.collection("chatMessages").doc(message.id).set({ ...message, module_id: moduleId, session_token: sessionToken, created_at: admin.firestore.FieldValue.serverTimestamp() });
    return (res as any).status(204).send();
});

// --- Authenticated Routes ---
router.post("/feedback", authed, async (req: AuthedRequest, res: express.Response) => {
    const { messageId, feedback } = (req as any).body;
    if (!messageId || !feedback) {
        return (res as any).status(400).json({ error: "messageId and feedback are required." });
    }
    await db.collection("chatMessages").doc(messageId).update({ feedback });
    return (res as any).status(204).send();
});

export default router;