// gemini-functions/src/routes/analyticsRoutes.ts
...
router.get("/tutor-logs/all", authed, async (req: AuthedRequest, res: express.Response) => {
    try {
        const logsSnap = await db.collection("tutorLogs").orderBy("created_at", "desc").limit(500).get();
        const logs = logsSnap.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
        return (res as any).status(200).json(logs);
    } catch (err) {
...
router.get("/question-frequency", authed, async (req: AuthedRequest, res: express.Response) => {
    const { moduleId } = (req as any).query;
    if (!moduleId) {
        return (res as any).status(400).json({ error: "moduleId is required." });
    }
    try {
        ...
        return (res as any).status(200).json(stats);
    } catch (err) {
...
router.get("/tutor-logs", authed, async (req: AuthedRequest, res: express.Response) => {
    const { moduleId } = (req as any).query;
    if (!moduleId) {
        return (res as any).status(400).json({ error: "moduleId is required." });
    }
...
router.get("/question-logs", authed, async (req: AuthedRequest, res: express.Response) => {
    const { moduleId, stepIndex, question, startDate, endDate } = (req as any).query;
    if (!moduleId || !stepIndex || !question) {
        return (res as any).status(400).json({ error: "moduleId, stepIndex, and question are required." });
    }
    try {
